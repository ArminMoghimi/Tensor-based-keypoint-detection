function ipts = SurfDescriptor_DecribeInterestPoints(ipts, upright, extended, img, lambda2,img2, verbose)
...
for i=1:length(ipts)
   ...
   [ip.orientation]=SurfDescriptor_GetOrientation(ip,img2,lambda2,verbose);

   % Extract SURF descriptor
   if(verbose), figure(h_des), subplot(10,4,i), end
   ip.descriptor=SurfDescriptor_GetDescriptor_weighted_image(ip, upright, extended,img2,lambda2,verbose);
