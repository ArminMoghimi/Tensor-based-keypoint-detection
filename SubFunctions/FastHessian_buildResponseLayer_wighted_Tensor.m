function rl=FastHessian_buildResponseLayer_wighted_Tensor(rl,FastHessianData)
...
% Compute response components
    for i=1:size(img,3)
        Dxx (:,i)= round(100.*FastHessianData.lambda(i)).*(IntegralImage_BoxIntegral(r - l + 1, c - b, 2 * l - 1, w,img(:,:,i)) - IntegralImage_BoxIntegral(r - l + 1, c - fix(l / 2), 2 * l - 1, l, img(:,:,i)) * 3);
        Dyy (:,i)= round(100.*FastHessianData.lambda(i)).*(IntegralImage_BoxIntegral(r - b, c - l + 1, w, 2 * l - 1,img(:,:,i)) - IntegralImage_BoxIntegral(r - fix(l / 2), c - l + 1, l, 2 * l - 1,img(:,:,i)) * 3);
        Dxy (:,i)= round(100.*FastHessianData.lambda(i)).*(+ IntegralImage_BoxIntegral(r - l, c + 1, l, l,img(:,:,i)) + IntegralImage_BoxIntegral(r + 1, c - l, l, l,img(:,:,i)) ...
...
rl.responses = (Dxx .* Dyy - 0.91 * Dxy .* Dxy);
